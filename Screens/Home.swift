...
struct Homiee: View {
    ...
    var temperaturaVMM = WeatherViewModel()
    ...
    var body: some View {
        ZStack {
            ...
            LinearGradient(gradient: Gradient(colors: self.temperaturaVMM.loadBackgroundImage ? [Color(#colorLiteral(red: 0.01843275475, green: 0.7711572018, blue: 0, alpha: 1)), Color(#colorLiteral(red: 0.5441120482, green: 0.5205187814, blue: 0.9921568627, alpha: 1))] : [Color(#colorLiteral(red: 0.1019607843, green: 0.168627451, blue: 0.262745098, alpha: 1)), Color(#colorLiteral(red: 0.3647058824, green: 0.5058823529, blue: 0.6549019608, alpha: 1))]), startPoint: .bottom, endPoint: .top)
                    ...
                    HStack(spacing: 32) {
                        ...
                        HStack {
                            ...
                            TextField("Enter city name", text: self.$searchField) {
                                self.temperaturaVMM.search(searchText: self.searchField)
//                                self.temperaturaVM.cityName = ""
                        ...
                        Button(action: {
                            self.temperaturaVMM.getWeatherByZipCode(by: self.zip, country_code: self.country_code)
                            self.showAlert = true
                        }) {
                    ...
                    HStack {
                        ...
                        VStack(alignment: .trailing, spacing: 5.0) {
                            ...
                            Text("\(self.temperaturaVMM.description.capitalized)")
                                .font(.title)
                                .bold()
                                .foregroundColor(.white)
                                .shadow(color: Color.black.opacity(0.20), radius: 4, x: 0, y: 3)

                            Text("\(self.temperaturaVMM.city_country)")
                                .foregroundColor(.white)
                                .shadow(color: Color.black.opacity(0.20), radius: 4, x: 0, y: 3)
                            Text("\(self.temperaturaVMM.date)")
                                .font(.footnote)
                                .foregroundColor(.white)
                                .shadow(color: Color.black.opacity(0.20), radius: 4, x: 0, y: 3)
                        }
                    ...
                    /// Temperature reading.
                    Text("\(self.temperaturaVMM.temperature)°")
                        .font(.system(size: 72))
                        .bold()
                        .foregroundColor(.white)
                        .shadow(color: Color.black.opacity(0.20), radius: 4, x: 0, y: 3)
 ...
 */
                        ...
                        HStack {
                            ...
                            Text("\(self.temperaturaVMM.sunset.replacingOccurrences(of: "PM", with: "")) pm")
                        }
                    }
                    .foregroundColor(.white)
                    .shadow(color: Color.black.opacity(0.20), radius: 4, x: 0, y: 3)
                    ...
                    VStack(spacing: 47) {
                        HStack(spacing: 5) {
                            DetailCell(icon: "thermometer.sun", title: "Humidity", data: self.temperaturaVMM.temperature, unit: "%")
                            Spacer()
                            DetailCell(icon: "tornado", title: "Wind Speed", data: self.temperaturaVMM.wind_speed, unit: "Km/hr")
                        }
                        
                        HStack(spacing: 5) {
                            DetailCell(icon: "arrow.down.circle", title: "Min Temp", data: self.temperaturaVMM.temp_min, unit: "°C")
                            Divider().frame(height: 50).background(Color.white)
                            DetailCell(icon: "arrow.up.circle", title: "Max Temp", data: self.temperaturaVMM.temp_max, unit: "°C")
                        }
...
struct Homiee_Previews: PreviewProvider {
